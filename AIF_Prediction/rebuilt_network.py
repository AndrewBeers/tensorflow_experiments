...
mse = tf.reduce_mean(tf.pow(y - outputs, 2))
average_error = tf.reduce_mean(tf.abs(y - outputs))

# Trainning with Adadelta Optimizer
train_step = tf.train.AdadeltaOptimizer(learning_rate=learning_rate).minimize(mse)
...
n_samples_train_test = [1000,1000]

trainset = np.array(ToftsSequenceData(n_samples=n_samples_train_test[0], max_seq_len=input_size).data)
testset = np.array(ToftsSequenceData(n_samples=n_samples_train_test[1], max_seq_len=input_size).data)

print X_train.shape
print np.array(y_train).shape
print trainset.shape

X_train = np.swapaxes(np.array(ToftsSequenceData(n_samples=n_samples_train_test[0], max_seq_len=input_size).data), 1, 2)
y_train = np.array(ToftsSequenceData(n_samples=n_samples_train_test[0], max_seq_len=input_size).labels)

X_test = np.swapaxes(np.array(ToftsSequenceData(n_samples=n_samples_train_test[1], max_seq_len=input_size).data), 1, 2)
y_test = np.array(ToftsSequenceData(n_samples=n_samples_train_test[0], max_seq_len=input_size).labels)
...
with tf.Session() as sess:
    ...
    for epoch in range(epochs):
        ...
        Loss = str(sess.run(mse, feed_dict={rnn._inputs: X, y: Y}))
    ...
    with open('rebuilt_outputs.csv', 'w') as csvfile:
